...
--       배정해, SLKO 직원이 키우자 교육자료를 보던 문제(RC1/RC2)를 코드에서 수정.
...
-- ── 1) 잘못 배정된 SLKO 직원 3명 + 그들의 모든 개인데이터를 Kiwooza → SLKO 로 이동 ──
...
-- basic_task_logs / employee_profiles / beta_feedback : SET company_id=SLKO WHERE employee_id IN (위 3명)
-- praise_logs : SET company_id=SLKO WHERE from_employee_id IN (3명) OR to_employee_id IN (3명)
COMMIT;

-- ── 2) Kiwooza 지점들을 branch_teams 에 백필 (가입 페이지가 anon 으로 읽는 유일한 소스) ──
--    회사별 지점 드롭다운이 브랜드별로 분리되도록. SLKO 는 이미 '본사' 행 존재.
...
-- 결과(적용 후): Kiwooza 14명 / SLKO 4명 · branch_teams 가 회사별로 분리(본사가 회사별 별도 행).
