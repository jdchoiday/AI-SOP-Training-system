...
[
  ...
  // ============================================
  // Pass 2 전용: 씬 배열 [...] 파싱, truncation 내성
  _parseScenesArray(cleaned) {
    ...
    if (lastBracket > 0) {
      const slice = body.slice(0, lastBracket + 1);
    ...
    // Try 2: 잘린 JSON — 기존 브래킷 복구기 재사용
    try {
      const repaired = this._repairAndParse(body);
      if (Array.isArray(repaired)) return repaired;
      return null;
    } catch (e) {
      console.warn('[Pass2 JSON] 복구 실패:', e.message);
      return null;
    }
  },
...

{
  ...
  "scene_count": 5~20 사이 (★원문 문장 수 ÷ 5로 산정★ — 한 씬 = 4~5문장 한 문단),
  "story_arc": [
...
- **scene_count는 원문 문장 수 ÷ 5 (★원문 문단 보존 원칙★)**:
  · **한 씬 = 4~5문장 한 문단** 개념
  · 원문 문장이 25개면 5씬, 50개면 10씬, 75개면 15씬 (★최대 20★)
  · 원문을 억지로 10씬으로 압축하지 말 것 — 문단이 많으면 씬도 많게
...
JSON만 출력, 마크다운/설명 금지.`;
    ...
    if (!plan.scene_count || plan.scene_count < 5) {
      // 문장 수 기반 추정: 마침표/물음표/느낌표/줄바꿈 단위, 한 씬 = 4~5문장
      const sentenceCount = (plainText.match(/[.!?。！？\n]+/g) || []).length || Math.ceil(plainText.length / 80);
      plan.scene_count = Math.min(20, Math.max(5, Math.ceil(sentenceCount / 5)));
    }
    // LLM이 scene_count를 반환했더라도 상한만 안전 가드 (20씬)
    if (plan.scene_count > 20) plan.scene_count = 20;
...
★ 원문 분배 원칙 (★중요★)
- 각 씬의 story_arc.idea / purpose 에 해당하는 **원문 문단**을 찾아서 그 문단의 문장들로 나레이션을 구성
- **원문을 순서대로 씬에 분배** — 1씬은 도입 문단, 마지막 씬은 마무리 문단
- 한 씬 = 원문의 4~5문장 한 문단 = 약 200~400자 분량
...
★ 나레이션 (TTS가 읽음)
- ${langInstr}
- **씬당 4~5문장 기준** (한 씬 = 원문의 한 문단 개념), 이모지·특수기호 금지
- **원문 보존 원칙 (★중요★)**:
  · 해당 씬에 배정된 원문 블록이 **5문장 미만이면 → 원문 그대로 사용 (수정·요약 금지)**
  · 해당 블록이 **5문장 이상이면 → 4~5문장으로 요약**하되 핵심 표현·어휘는 살림
  · 요약하더라도 원문의 어휘·문장 구조를 가능한 한 유지하고, 의미를 바꾸지 말 것
- 핵심 메시지를 중복 없이 분배
...
JSON 배열만 출력:`;
    ...
    let scenes = this._parseScenesArray(cleanedP2);
    if (!scenes || scenes.length === 0) {
      throw new Error('Pass 2 JSON 파싱 실패 (응답 앞: ' + cleanedP2.slice(0, 200) + ' / 뒤: ' + cleanedP2.slice(-200) + ')');
    }
